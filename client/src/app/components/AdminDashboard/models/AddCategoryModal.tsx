...
const AddCategoryModal: React.FC<Props> = ({ open, onClose, onAddCategory, selectedCategory, onUpdateCategory }) => {
    ...
    return (
        <>
            <Snackbar open={isSnackbarOpen} autoHideDuration={4000} onClose={() => setIsSnackbarOpen(false)} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}>
                <Alert severity="error" onClose={() => setIsSnackbarOpen(false)}>{snackbarMessage}</Alert>
            </Snackbar>
            <Dialog open={open} fullWidth onClose={onClose}>
                ...
                <DialogContent className='bg-dark-surface border-primary-variant border-4'>
                    ...
                    <TextField name="description" label="Description" fullWidth multiline rows={4} value={categoryData.description} onChange={handleChange} error={!!error.description} helperText={error.description || ''} />
