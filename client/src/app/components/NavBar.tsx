	...
	return (
		<nav className="bg-white mx-auto max-w-7xl">
			...
			<div className="hidden md:flex justify-between items-center h-28 ">
				...
				<div className="flex items-center justify-center flex-1">
					<Link href="/">
						<Herba
							name="Herba"
							className="herbaIcon h-9/12 w-9/12 fill-primary"
						/>
					</Link>
				</div>
				<div className="flex items-center justify-end flex-1 gap-4 mr-4">
					...
					>
						<AccountIconLocal
							...
							width={"40"}
							height={"40"}
							className="accountIcon  hover:text-primary-variant hover:scale-110 transition duration-300 text-black"
						/>
					</IconButton>
					<Menu
					...
					>
						...
						<MenuItem onClick={handleAuthAction}>{isLoggedIn ? "Logout" : "Login"}</MenuItem>
					</Menu>
					<IconButton
						aria-label="button"
					...
					>
						<CartIcon
							...
							width={"40"}
							height={"40"}
							className="cartIcon  hover:text-primary-variant hover:scale-110 transition duration-300 text-black"
						/>
					</IconButton>
				</div>
			</div>
			...
			<ul className="hidden sm:flex justify-evenly py-2 sm:text-xs md:sm lg:text-base xl:text-lg ">
				{navLinks.map((link, i) => (
					...
					>
						<button
							type="button"
							onClick={() => router.push(`/shop${link.href}`)}
						>
			...
			<div className="md:hidden  flex justify-between items-center py-2 px-4 h-24">
				...
				>
					{/* Top line */}
					<div className={`w-8 h-1 bg-black transition duration-300 ease-in-out ${isMobileMenuOpen ? "rotate-45 translate-y-3" : ""}`}></div>

					{/* Middle line */}
					<div className={`w-8 h-1 bg-black transition duration-300 ease-in-out ${isMobileMenuOpen ? "opacity-0" : ""}`}></div>
				...
				<Herba
					...
					width={"375"}
					height={"175"}
					className="herbaIcon h-full w-full fill-primary"
				/>
				<div className="flex items-center justify-end flex-1 gap-4">
					<Link href="/account">
						<AccountIconLocal
							...
							width={"40"}
							height={"40"}
							className="accountIcon hover:text-primary-variant hover:scale-110 transition duration-300"
						/>
					</Link>
					<button
						type="button"
					...
					>
						<CartIcon
							...
							width={"40"}
							height={"40"}
							className="cartIcon hover:text-primary-variant hover:scale-110 transition duration-300"
						/>
					</button>
				</div>
			</div>
			...
			{isMobileMenuOpen && (
				<ul className="md:hidden">
					{navLinks.map((link) => (
						...
						>
							<Link href={link.href}>
								<span className="block p-4 text-sm uppercase font-semibold text-primary hover:primary-variant">{link.label}</span>
							</Link>
						</li>
					))}
				</ul>
