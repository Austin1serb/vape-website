			...
			<div className={`${styles.formContainer} ${styles.signUp}`}>
				<form onSubmit={handleRegister}>
					...
					<div className="w-full">
						...
						/>
						<Collapse
							in={(focus && progress !== 1) || !!regError}
							unmountOnExit
						>
							{focus && progress !== 1 ? (
								<div className="h-[70px] text-xs text-red-600">Password must be at least 8 characters long, and contain at least one number and one letter.</div>
							) : (
								<div className="h-[70px] text-xs text-red-600">{regError?.email ?? regError?.firstName ?? regError?.lastName ?? ""}</div>
							)}
						</Collapse>
					</div>
			...
			<div className={`${styles.formContainer} ${styles.signIn}`}>
				<form onSubmit={handleLogin}>
					...
					<Collapse
						...
						sx={{ width: "100%" }}
					>
						<div className="text-xs text-red-600 w-full text-start">{error}</div>
					</Collapse>

					<a
						href="#"
						className="text-primary hover:text-secondary-variant transition duration-300"
					>
