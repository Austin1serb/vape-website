...
const Cart: React.FC<{ setDrawerOpen: React.Dispatch<React.SetStateAction<boolean>> }> = ({ setDrawerOpen }) => {
                ...
                <Box sx={{ maxHeight: '415px', overflowY: 'auto', }}>
                    {cart.length === 0 ? (
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: "center", height: { xs: 200, sm: 300 } }}>
                            ...
                            }} href="/shop" component={Link} sx={{ border: 1, width: '90%', borderRadius: 0, letterSpacing: 2, color: '#283047', backgroundColor: 'white', borderColor: '#283047', borderWidth: 1.5, height: 55, '&:hover': { backgroundColor: '#0F75E0', color: 'white' } }} > Start Shopping</Button>
                        </Box>
                    ) : (
                        <List>
                            {cart.map(item => (
            ...
            <Box>
                ...
                <Box sx={{
                    ...
                    <Box className="ajax-cart--bottom-wrapper" sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        {cart.length > 0 && (
                            <>
                                <Button component={Link} href='/checkout/1' onClick={handleProceedToCheckout} variant="outlined" sx={{ width: { xs: '75%', sm: '90%', md: '90%' }, letterSpacing: 2, color: 'white', borderRadius: 0, backgroundColor: '#283047', mb: { xs: 5, sm: 3 }, height: 56.5, "&:hover": { backgroundColor: '#FE6F49', border: 'none', }, textAlign: 'center' }}>Place your order</Button>
                                <Button onClick={() => {
                                    setDrawerOpen(false); // Close the drawer
