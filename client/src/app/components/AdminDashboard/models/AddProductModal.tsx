...
import { Button, Dialog, DialogContent, DialogContentText, DialogTitle, CircularProgress, Snackbar, Alert, SelectChangeEvent } from "@mui/material";
...
const AddProductModal: React.FC<Props> = ({ open, onClose, onAddProduct, selectedProduct, onUpdateProduct, brands, categories }) => {
	...
	const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/api/product/` || "http://localhost:8000/api/product/";
	...
	const parseGptResponse = (response: any) => {
		const { description, seoTitle, seoDescription, seoKeywords } = response;
	...
	return (
		<>
			...
			<Dialog
				...
				fullWidth
			>
				<DialogTitle className="bg-primary-variant text-center uppercase h-18">{selectedProduct ? "Edit Product Details" : "Add New Product"}</DialogTitle>

				<DialogContent
					sx={{ borderRadius: 3 }}
					className="bg-dark-surface border-primary-variant border-4"
				>
					<DialogContentText sx={{ my: 2 }}>Please fill in the details of the new product.</DialogContentText>
