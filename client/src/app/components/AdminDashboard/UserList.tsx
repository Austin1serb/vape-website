...
const UserList: React.FC<UsersState> = ({ customers, guests }) => {
	...
	const confirmDeleteUser = async () => {
		...
		try {
			...
			setUsers((prev) => {
				...
				const updatedCustomers = userTypeToDelete === "customer" ? prev.customers.filter((customer) => customer._id !== userToDelete) : prev.customers;
				const updatedGuests = userTypeToDelete === "guest" ? prev.guests.filter((guest) => guest._id !== userToDelete) : prev.guests;
	...
	const handleMakeAdmin = (customerId: string) => {
		fetchData(`${BASE_URL}/api/customer/${customerId}`, {
			...
			.then((updatedCustomer) => {
				setUsers((prevUsers) => ({
					...prevUsers,
					customers: prevUsers.customers.map((customer) => (customer._id === customerId ? { ...customer, isAdmin: true } : customer)),
				}));
			})
			.catch((error) => {
		...
		fetchData(`${BASE_URL}/api/customer/${adminId}`, {
			...
			.then(() => {
				setUsers((prevUsers) => {
					...
					return {
						...prevUsers,
						customers: prevUsers.customers.map((customer) => (customer._id === adminId ? { ...customer, isAdmin: false } : customer)),
					};
				});
			})
			.catch((error) => {
	...
	const updateCustomerInList = (updatedCustomer: Customer) => {
		setUsers((prevUsers) => ({
			...prevUsers,
			customers: prevUsers.customers.map((customer) => (customer._id === updatedCustomer._id ? updatedCustomer : customer)),
		}));
	};
